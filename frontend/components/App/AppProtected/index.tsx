...
import { Data, data as data_1, IndexProps, Result } from '../../../interfaces';
import green from '@mui/material/colors/green';
import getContractData from '../../apis/contractdata';
import { Mint } from '../Mint';
import orange from '@mui/material/colors/orange';

export function AppProtected (props: IndexProps) {
  ...
  React.useEffect(() => {
    ...
    const getBalance = async() => {
      ...
      if(provider) {
        ...
        setBalance(result.balanceOrAllowance.toString());
      }
    }
        ...
        <Grid container spacing={2}>
          ...
          </Grid>
          {
            !isUser? <Grid item xs={isUser? 3 : 12}>
              <Box sx={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
                <Mint handleClick={handleClick} setauth={setauth} isUser={isUser} address={state?.address} isConnected={state?.isUserAuthenticated} provider={state?.provider} />
              </Box>
          ...
          <Grid item xs={12} md={6}>
            <CardComponent
              ...
              description={`Test balance: ${balance} ${'CELOG'}`}
            >
              <Spinner color={'white'} />
            </CardComponent>
          </Grid>
