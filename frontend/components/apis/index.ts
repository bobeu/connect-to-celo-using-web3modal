...
function contractInstances(props: InstanceProps) {
  ...
  const testToken2 = new Contract(memberAddr, memberAbi, new ethers.providers.Web3Provider(providerOrSigner).getSigner());

  return { swapLab, swapLab_noSigner, token, token_noSigner, nft, testToken2 }
}
...
async function runContractFunc(options: OptionProps) {
  ...
  const { swapLab, nft, token, testToken2 } = contractInstances({
    swapAbi,
    test2Addr,
    memberAbi,
    memberAddr,
  ...
  switch (functionName) {
    ...
    case 'claim':
      ...
      await txn_2?.wait(2).then(async() => {
        const xTrx = await testToken2.selfClaimDrop();
        await xTrx?.wait(2).then(async(rec: ContractReceipt) => {
          if(rec) {
    ...
    case 'getBalanceToken2':
      result.balanceOrAllowance = await getBalanceTestToken2(account);
      break;
